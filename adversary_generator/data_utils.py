...
def load_twitter_gender_data(from_cache=False):
    ...
    #raw_data = pd.read_csv(raw_data_path, encoding='latin1', nrows = 50000)
    ...
    #     'index':index,
    #     'SentimentText':tweet,
    #     'Sentiment':raw_data['Sentiment'][index]
    #     })
    #
    # with open('./data/clean_data.csv', 'w') as handle:
    #     writer = csv.DictWriter(handle,
    #             fieldnames=cleaned_text_data[0].keys())
    #     writer.writeheader()
    #     for item in cleaned_text_data:
    #         writer.writerow(item)

    raw_data = pd.read_csv('./data/clean_data.csv', encoding='latin1', nrows = 50000)
