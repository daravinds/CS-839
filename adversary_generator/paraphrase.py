...
def perturb_text(
    ...
    Insertion/Removal of adjective or adverb is done post typo/synonym change.
    Default mode is 3.
    ...
    if not use_typos and not use_synonyms:
        print('Extending words')
    ...
    # For mode 4. order of change
    # 2 CHANGES (Sorted by score - could be any) + 1 Insertion/Removal (if possible)

    while len(sorted_candidates) > 0 and not halt_condition_fn(perturbed_text) and counter < changes and (use_typos or use_synonyms):
        score, candidate = sorted_candidates.pop()
    ...
    if not halt_condition_fn(perturbed_text) and mode != 2:
        print ("Inserting/Removing word")
