...
nlp = spacy.load('en_core_web_md', tagger=False, entity=False)
...
def adversarial_paraphrase(doc, grad_guide, target, max_length=1000,
                           use_typos=False, use_synonyms=False, verbose=False, mode=3):
    '''
    ...
    if verbose:
        print('Confidence before change', y)
    ...
    perturbed_text = perturb_text(doc,
            ...
            verbose = verbose, target = target, mode = mode)

    perturbed_x = extract_features([nlp(perturbed_text)],
            max_length=max_length).reshape(1, -1)
    perturbed_y = model.predict(perturbed_x, verbose=0).squeeze()
    _stats_probability_shifts.append(math.fabs(perturbed_y - y))

    if verbose:
        print('Confidence after change:', perturbed_y)
